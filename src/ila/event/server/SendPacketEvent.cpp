...
void ISendPacketBeforeEvent::deserialize(CompoundTag const& nbt) {
    senderSubId() = magic_enum::enum_cast<SubClientId>(nbt["senderSubId"].get<StringTag>()).value_or(senderSubId());
}
...
LL_TYPE_INSTANCE_HOOK(
    ...
    eventBus.publish(beforeEvent, [&]() -> ll::event::EventId {
        ...
        return ll::event::EventId{
            fmt::format("{0}<class {1}>", ll::reflection::type_name_v<SendPacketBeforeEvent<Packet>>, packetName)
        };
    }());
    ...
    eventBus.publish(afterEvent, [&]() -> ll::event::EventId {
        ...
        if (!packetName.ends_with("Packet")) packetName += "Packet";
        return ll::event::EventId{
            fmt::format("{0}<class {1}>", ll::reflection::type_name_v<SendPacketAfterEvent<Packet>>, packetName)
        };
    }());
}
    ...
    static inline bool reg = []() -> bool {
        constexpr static auto addEvent = [](std::string const& eventName) -> void {
            LLEventBus.setEventEmitter(
                ...
                ll::event::EventId{
                    fmt::format("{0}<class {1}>", ll::reflection::type_name_v<SendPacketBeforeEvent<Packet>>, eventName)
                }
            );
            LLEventBus.setEventEmitter(
                SendPacketEventEmitterFactory,
                ll::event::EventId{
                    fmt::format("{0}<class {1}>", ll::reflection::type_name_v<SendPacketAfterEvent<Packet>>, eventName)
                }
            );
        };
