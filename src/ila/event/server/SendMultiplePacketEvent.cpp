#include "ila/event/server/SendMultiplePacketEvent.h"
...
optional_ref<ServerPlayer>
ISendMultiplePacketBeforeEvent::player(NetworkIdentifierWithSubId const& networkIdentifier) const {
...
optional_ref<ServerPlayer>
ISendMultiplePacketAfterEvent::player(NetworkIdentifierWithSubId const& networkIdentifier) const {
    return ll::service::getServerNetworkHandler()->_getServerPlayer(
        networkIdentifier.id,
        networkIdentifier.subClientId
    );
...
LL_TYPE_INSTANCE_HOOK(
    ...
    eventBus.publish(afterEvent, [&]() -> ll::event::EventId {
        ...
        return ll::event::EventId{
            fmt::format("{0}<class {1}>", ll::reflection::type_name_v<SendMultiplePacketAfterEvent<Packet>>, packetName)
        };
    }());
}
...
public:
    ...
    ~SendMultiplePacketEventEventEmitter() { ll::memory::HookRegistrar<SendMultiplePacketEventHook>().unhook(); }
};
static std::unique_ptr<ll::event::EmitterBase> SendMultiplePacketEventEmitterFactory() {
