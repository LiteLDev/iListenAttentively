...
public:
    constexpr explicit ActorRideBeforeEvent(Actor& actor, Actor& target) : Cancellable(actor), mTarget(target) {}
...
public:
    constexpr explicit ActorRideAfterEvent(Actor& actor, Actor const& target) : ActorEvent(actor), mTarget(target) {}
