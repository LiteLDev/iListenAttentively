...
LL_TYPE_INSTANCE_HOOK(ActorRideEventHook, HookPriority::Normal, Actor, &Actor::$addPassenger, void, Actor& pPassenger) {
    if (!canAddPassenger(pPassenger)) {
        return origin(pPassenger);
    }
    ...
    origin(pPassenger);
    if (isPassenger(pPassenger)) {
        LLEventBus.publish(ActorRideAfterEvent(pPassenger, *this));
    }
}
