...
#define Event_Hook_Factory_Base(eventName, ...)                                                                        \
    ...
    class eventName##EventEmitter : public ll::event::Emitter<eventName##EmitterFactory, eventName##Event> {           \
    ...
    public:                                                                                                            \
        ...
        ~eventName##EventEmitter() {                                                                                   \
            for (auto& listener : mListeners) {                                                                        \
                LLEventBus.removeListener(listener);                                                                   \
            }                                                                                                          \
            mListeners.clear();                                                                                        \
        }                                                                                                              \
    };                                                                                                                 \
...
template <>
_NODISCARD_SMART_PTR_ALLOC inline std::shared_ptr<BlockSourceHandle>
std::make_shared<BlockSourceHandle, BlockSource&>(BlockSource& region) {
